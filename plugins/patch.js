...
smd({
  ...
  try {
    ...
    if (url.toLowerCase().includes("https")) {
      ...
      return await message.send("`LEGEND-MD UPDATE WAS SUCCESSFULL ✅`");
    } else {
...
astro_patch.smd({
  ...
  try {
    ...
    if (menuThemeType === 1 || Config.menu.trim().startsWith("1") || Config.menu.toLowerCase().includes("menu1")) {
      ...
      menuThemeHeader = " ----0:*" + Config.botname + "*:o--\n┃⥏╭──────────────";
      menuThemeCommandPrefix = "┃⥏│";
      menuThemeFooter = "┃⥏╰───────────────\n╰═════════════════⊷";
      menuThemeCategoryHeader = "╭─❏";
      menuThemeCategoryFooter = "❏";
...
astro_patch.smd({
  ...
  await context.send(`*🐣...latency of LEGEND is....🐤: ${pingTime} ᴍs*`, {
    edit: messageKey
  }, "", context);
});
astro_patch.smd({
    ...
    const endTime = new Date().getTime();
    const pingTime = endTime - startTime;
    await context.send(`*LEGEND IS ACTIVE....🐤*`, {
      edit: messageKey
    }, "", context);
  });
smd({
  ...
  try {
    ...
    const designs = [async () => {
      const imageBuffer = await axios.get("https://raw.githubusercontent.com/HyHassan/HyHassan/main/Images/logo.jpg", {
...
smd({
  ...
  try {
    ...
    const contextInfo = {
      isForwarded: true,
      forwardingScore: 999,
      title: "LEGEND Running Since",
      body: `*LEGEND-MD Running Since:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`,
      footerText: "LEGEND || TalkDrove",
      isSendNotificationMsg: true,
      mentionedJid: []
    };
    await m.bot.sendMessage(m.chat, {
      text: `*LEGEND-MD Running Since:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`,
      contextInfo
    }, {
      quoted: null
    });
...
smd({
  ...
  try {
    ...
    if (!isNaN(_0x56b5d3)) {
      if (_0x1eb88a.length > 30 && _0x1eb88a[1].includes("LEGEND-MD_FANCY_TEXT")) {
      ...
      if (_0x56c097.length > 0 && _0x56c097.length < 20) {
        ...
        for (const _0x1af79d in _0x59e793) {
          ...
          if (_0x56c097.includes(_0x37f2ac)) {
            _0x5cca14 = "┏━━━━━━━━━━━━━━━━━━━━━━━\n┃\tLEGEND-MD_" + _0x1af79d.toUpperCase() + "_COMMANDS*  \n┗━━━━━━━━━━━━━━━━━━━━━━━\n\n\n";
