...
smd(
 {
   pattern: "channel",
   desc: "To check channel",
   react: "🗨️",
   category: "user",
   filename: __filename,
 },
 async (message) => {
   const channelMessage = `LEGEND-MD CHANNEL\n *LINK:* https://whatsapp.com/channel/0029VajDL6mHFxP88HXtWG3F\n\n ${Config.botname} *WORKS*`;
...
smd(
 {
   pattern: "wagroup",
   desc: "TO GET SUPPORT NUMBER OF Hassan",
   react: "🗨️",
   category: "user",
   filename: __filename,
 },
 async (message) => {
   const SupportMsg = `LEGEND-MD WHATSAPP GROUP\n\n *ʟɪɴᴋ:*https://chat.whatsapp.com/DkHKZZ7qBjkBZDqSJLG6tL \n\n Powered by ${Config.botname}`;
...
smd({
  ...
  try {
    let _0x12a593 = "*HEY LEGEND X HM IS CURRENTLY ACTIVE! 👾*";
...
smd({
  ...
  try {
    ...
    if (!_0x11eeb1) {
      return await _0x7587f6.reply("_Need fileName, Example: document Hassan | caption_");
    }
...
smd({
  ...
  try {
    ...
    if (!_0x587b99) {
      return await _0x4cdec8.reply("*Need URL! Use " + prefix + "ss https://github.com/HsnCyber/LEGEND-X-HM*");
    }
