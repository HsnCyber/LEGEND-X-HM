...
smd(
 ...
 async (m, username) => {
   try {
     if (!username) {
       return await m.send("*_Please provide a GitHub Username! e.g HyLegend x HM_*");
     }
      ...
      if (!query || !query.startsWith("https://")) {
        return await message.send(
          "*_Please provide a valid Twitter Video URL._*\n*Example: " +
            prefix +
            "tw https://twitter.com/username/HyLegend x HM*"
        );
      }
    ...
    try {
      ...
      for (let i = startIndex - 1; i < endIndex; i++) {
        ...
        if (isSticker) {
          ...
          await message.reply(
            sticker,
            { packname: Config.packname, author: "Legend x HM-Md" },
            "sticker"
          );
        } else {
    ...
    try {
      ...
      if (!_0x1c586e) {
        return await _0x1ae8f8.reply(
          "*Provide Repo Url, _.gitclone https://github.com/HyLegend x HM/Legend x HM-MD_*"
        );
      }
      const _0x5906ab =
...
smd({
  ...
  try {
    ...
    if (!_0x204f81) {
      return _0x55aba2.reply("*_Example : .tts Hi,I am Legend x HM-Md whatsapp bot._*");
    }
    try {
      ...
      await _0x55aba2.bot.sendMessage(_0x55aba2.jid, {
        ...
        mimetype: "audio/mpeg",
        ptt: true,
        fileName: "Legend x HM-Md-tts.m4a"
      }, {
        quoted: _0x55aba2
      });
    } catch (_0x3537cb) {
...
smd({
  pattern: "sound",
   alias: ["Legend x HM", "aine","mentalism","alive","waso"],
...
smd(
  ...
  async (message, url) => {
    try {
      ...
      const apiUrl = "http://widipe.com/download/tiktokdl?url=";
...
smd(
  ...
  async (message, url) => {
    try {
      ...
      const apiUrl = `http://widipe.com/download/tiktokdl?url=${encodeURIComponent(
        tiktokUrl
      )}`;
      const response = await fetchJson(apiUrl);
...
smd(
  ...
  async (m, url) => {
    try {
      ...
      const caption = `『 *Legend x HM MD Mᴇᴅɪᴀғɪʀᴇ Dᴏᴡɴʟᴏᴀᴅᴇʀ* 』\n\n *Name* : ${result.name}\n *Size* : ${result.size}\n *Mime* : ${result.mime}\n\n\n${Config.caption}`;
...
cmd({
  ...
  if (_0xb75e78.quoted && _0xb75e78.text) {
    ...
    if (_0x5b8ee5[0].includes("Legend x HM MD || sᴏɴɢ ᴅᴏᴡɴʟᴏᴀᴅᴇʀ")) {
...
smd({
 ...
 try {
   ...
   try {
     ...
     let _0x452f00 = _0x5c3baf.title || _0x57ada1 || _0x13f167[0x1] || "Legend x HM MD -- YT Video";
     if (_0x57ada1) {
