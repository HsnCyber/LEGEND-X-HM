...
Database:  ${(isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "LEGEND Default")}
\t_VARS_
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^wasi_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/Legend;;;/gi, "").replace(/LEGEND;;;/gi, "").replace(/Byte;;;/gi, "").trim();
